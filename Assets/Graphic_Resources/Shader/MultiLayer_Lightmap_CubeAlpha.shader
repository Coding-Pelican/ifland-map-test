//멀티레이어 쉐이더 - 라이트맵용/기존 쉐이더포지로 제작된 시크릿타운 라이트맵과 큐브맵 부분 + 알파 쉐이더로 수정 //
//알파 - Transparent /큐브맵 부분 알파 제거는 제작 중
//현재는 전체가 알파를 먹는 형식

//-----SecretTown------//
Shader "Secrettown/MultiLayer_Lightmap_CubeAlpha"
{
    Properties
    {
//------------------CubeMap 텍스쳐 공통으로 적용--------------------//
...
[Header(Other Texture)]
        _Cutoff04 ("Alpha cutoff", Range(0,1)) = 0.5
...
//----------------------------------------------------------------------------------//
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed _Cutoff01, _Cutoff02, _Cutoff03, _Cutoff04;
            ...
            fixed4 frag (VertexOutput i) : SV_Target
            {
				...
				if ( i.uv1.x > 0.0 )
				{
					...
					fixed4 finalRGBA = fixed4(finalColor,  _Cutoff01);
				...
				else if ( i.uv2.x > 0.0 )
				{
					...
					//float3 finalColor = (diffuseColor * _DiffuseStrength) + emissive;
					fixed4 finalRGBA = fixed4(finalColor,_Cutoff02);
				...
				float3 finalColor = (diffuseColor * _DiffuseStrength)   + emissive;
					...
					fixed4 finalRGBA = fixed4(finalColor,_Cutoff03);
					UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
				...
				else if ( i.uv4.x > 0.0 )
				{
					...
					//float3 finalColor = (diffuseColor * _DiffuseStrength) + emissive;
					fixed4 finalRGBA = fixed4(finalColor,_Cutoff04);
					UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
